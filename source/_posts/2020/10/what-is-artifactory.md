...
我在使用 Artifactory 已经有挺长一段时间了，本篇是我对使用 Artifactory 做持续集成的实践分享。包括企业级 Artifactory 有哪些优势，如何使用，以及工作流、仓库命名、目录结构、仓库权限、保留策略。
...
可能你的团队已经有了自己的管理制品的方式，比如 FTP。Artifactory能带来什么呢？让我来给你介绍一下它有哪些优势。
...
如果你的构建是存储在 FTP 上，你想从大量的制品中找到你要找的那一个，如果你还不知道它的名字，那么真的很难找到它。
...
接下来我通过一个 Demo 来介绍 Artifactory 应该怎么使用，以及其中有哪些最佳实践，避免踩坑。
...
你可以看到这个 Artifactory 已经服务了超过5000件的制品。然后可以看到 Artifactory 的当前版本号，以及最新版本。

### 页面中部，从左到右

1. 最左边是搜索功能，通过丰富的搜索条件可以轻松找到制品。然后是一些用户手册、视频、REST API 文档等信息。

2. 中间是 Set Me Up，使用它可以选择和筛选你想要操作的存储库，单击特定的存储库可以弹出关于如何使用它的详细说明。
...
在首页的 Set Me Up 里你也看到了我们有很多仓库（Repository），然而在众多仓库中，你只对其中一些仓库感兴趣，那么就可以只关注部分仓库。添加喜欢，然后点击喜欢按钮就可以只列出你关注的 Artifact Repository 了。
...
这个过程中，就像使用漏斗一样将构建通过层层帅选，从 `int` 仓库到 `stage` 仓库，最后到 `release` 仓库完成了制品的发布。
...
以上我就是我使用企业级 Artifactory 的实践分享。你如有还有什么想法和建议，欢迎留言交流。
