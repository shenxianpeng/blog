---
title: Jenkins 构建策略的既要又要还要，一段代码搞定
tags:
  - Jenkins
  - DevOps
categories:
...
在我们 Jenkins 流水线中经常会遇到这样一个情况：同一个分支或PR，如果有一个构建任务正在运行，当有新的 commit 进来时，当前的构建任务会自动被中断。

原因是我们需要考虑资源使用的问题，特别是对于那些通常需要较长时间才能完成的构建任务。如果允许同一个分支或者同一个 PR 同时进行多个构建，资源很快就会被占用殆尽，后面的只能排队。
...
![What's the different?](jenkins-concurrent-build-cn/1.png)
