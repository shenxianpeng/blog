...
6. **Generate the cover:** `python3 .github/scripts/gen_cover.py <post-dir>`.
   Do this after the tags are final — the palette is keyed to the primary tag.
   See "Images".
...
**Generate it, do not hunt for one:**

```bash
python3 .github/scripts/gen_cover.py content/posts/2026/your-slug
```

Covers across the site are generated by that script and share one visual
language: an abstract geometric composition — pipeline, orbit, bars or contours
— on a dark ground. Output is deterministic, so the same post always produces
the same cover and re-running it never churns the repo. The palette is keyed to
the post's **primary tag**, which groups a topic into one colour family on list
pages; the composition is keyed to the post path, so two posts sharing a tag
still look different. Set the tags before generating.

This replaced a set where 259 posts shared just 73 images — one illustration was
doing duty on 54 posts at once.

If you supply a cover by hand instead:
...
- Never reuse another post's cover as filler. Generate one instead; it costs a
  second.
